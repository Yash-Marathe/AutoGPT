...
def test_turns_and_results(
    battleship_game: typing.Any, initialized_game_id: str
) -> None:
...
def test_game_status_and_winner(battleship_game: typing.Any) -> None:
    game_id = battleship_game.create_game()
    status = battleship_game.get_game_status(game_id)
    assert isinstance(status.is_game_over, bool), f"Invalid game status: {status}"
    if status.is_game_over:
        winner = battleship_game.get_winner(game_id)
        assert winner is not None, f"Winner is None: {winner}"


def test_delete_game(battleship_game: typing.Any) -> None:
    game_id = battleship_game.create_game()
...
def test_ship_rotation(battleship_game: typing.Any) -> None:
    game_id = battleship_game.create_game()
    ...
    game = battleship_game.get_game(game_id)
    assert placement_horizontal in game.ships, f"Horizontal ship not found: {placement_horizontal}"
    assert placement_vertical in game.ships, f"Vertical ship not found: {placement_vertical}"


def test_game_state_updates(battleship_game: typing.Any, initialized_game_id: str) -> None:
    turn = Turn(target={"row": 3, "column": "A"})
    battleship_game.create_turn(initialized_game_id, turn)

    game = battleship_game.get_game(initialized_game_id)
    target_key = (3, ord("A") - ord("A"))
    assert target_key in game.board, f"Target key not found in board: {target_key}"
    assert game.board[target_key] == "hit" or game.board[target_key] == "miss"


def test_ship_sinking_feedback(battleship_game: typing.Any, initialized_game_id: str) -> None:
    hits = ["A", "B", "C", "D"]
    static_moves = [
        {"row": 1, "column": "E"},
        {"row": 1, "column": "F"},
    ...
    assert response.result == "sunk", f"Result should be 'sunk', not {response.result}"


def test_restart_game(battleship_game: typing.Any) -> None:
    game_id = battleship_game.create_game()
    battleship_game.delete_game(game_id)
    game_id = battleship_game.create_game()
    game = battleship_game.get_game(game_id)
...
def test_ship_edge_overlapping(battleship_game: typing.Any) -> None:
    game_id = battleship_game.create_game()
    ...
    game = battleship_game.get_game(game_id)
    assert first_ship in game.ships, f"First ship not found: {first_ship}"
    assert next_ship in game.ships, f"Next ship not found: {next_ship}"
